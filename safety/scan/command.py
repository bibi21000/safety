...
from safety.scan.render import print_brief, print_detected_ecosystems_section, print_fixes_section, print_ignore_details, render_scan_junit, render_scan_html, render_scan_spdx, render_to_console
...
def process_report(obj: Any, console: Console, report: ReportModel, output: str,
    ...
    with console.status(wait_msg, spinner=DEFAULT_SPINNER) as status:
        ...
        if export_type is ScanExport.HTML or output is ScanOutput.HTML:
            html_format = render_scan_htmlrender_scan_html(report, obj)

        if export_type is ScanExport.JUNIT or output is ScanOutput.JUNIT:
            junit_format = render_scan_junit(report, obj)
